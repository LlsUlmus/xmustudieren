<template>
    <component :is="'script'" ref="script" :name="name" type="text/plain"></component>
</template>

<script>
import LoadEvent from '../utils/Event.js';
import Debounce from '../utils/Debounce.js';
import UEditorUploadHelper from '../utils/ImageHandler.js';
import { accessToken as loginToken } from '@/cores/security/useCurrentUser';

export default {
    name: 'VueUeditorWrap',
    data() {
        let autoFloatEnabled = this.topOffset !== null;
        return {
            status: 0,
            initValue: '',
            defaultConfig: {
                // VUE CLI 3 会添加 process.env.BASE_URL 的环境变量，而 VUE CLI 2 没有，所以借此设置 UEDITOR_HOME_URL，能涵盖大部分 Vue 开发者的使用场景
                UEDITOR_HOME_URL: import.meta.env.MODE !== "production" ? 'http://cdn.ricebird.cn/static/UEditor/' : '/static/UEditor/',
    ...
    methods: {
        ...
        _initEditor() {
            ...
            if (this.height) {
                this.mixedConfig.initialFrameHeight = this.height;
            }
            this.editor = window.UE.getEditor(this.id, this.mixedConfig);
            const ueditor = this.editor;
            this.editor.addListener('ready', () => {
                const { requestKey, token } = loginToken;
                this.editor.execCommand('serverparam', requestKey, token);
                let helper = UEditorUploadHelper(this.editor);
                this.editor.UploadHelper = helper;
                if (this.readonly) {
    ...
    watch: {
        ...
        readonly: {
            handler(newValue) {
                if (this.editor) {
                    newValue ? this.editor.setDisabled() : this.editor.setEnabled();
                }
            },
            immediate: true // 确保组件初始化时也检查一次 readonly 的值
        }
    }
};
</script>
