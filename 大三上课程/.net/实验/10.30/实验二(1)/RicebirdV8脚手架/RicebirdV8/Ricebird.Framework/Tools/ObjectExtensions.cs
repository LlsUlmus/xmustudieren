...
namespace Ricebird.Framework
{
    public static class ObjectExtensions
    {
        ...
        {
            if (obj is ExpandoObject or Dictionary<string, object>)
            {
        ...
        public static T ObjectToObject<T>(this object obj)
            where T : new()
        {
            T dist = new();
            foreach (var prop in obj.GetType().GetProperties())
            {
                PropertyInfo? tProp = dist.ContainName(prop.Name);
                if (tProp != null)
                {
                    var v = prop.GetValue(obj);
                    var value = ValueUtils.ChangeToType(v, tProp.PropertyType, new object());
                    tProp.SetValue(dist, value);
                }
            }
            return dist;
        }

        public static void CopyFromObject(this object dest, object src)
        {
            CopyFromObject(dest, src, string.Empty);
        }

        public static void CopyFromObject(this object dest, object src, params string[] ignoreProperties)
        {
            Utils.CopyPropertiesFrom(dest, src, ["CreatedOn", .. ignoreProperties]);
            ...
            //{
            //    var destProp = dstType.GetProperty(prop.Name);
            //    try
            //    {
            //        if (destProp != null && destProp.CanWrite && destProp.CanRead
            //            && !ignoreList.Contains(prop.Name))
            //        {
            //            object? val = null;
            //            val = prop.GetValue(src, null);
            //            var value = ValueUtils.ChangeType(val, destProp.PropertyType, new object());
        ...
        public static void CopyFromClient(this object dest, IClient client)
        {
            Type type = dest.GetType();
            foreach (var prop in type.GetProperties())
            {
                if (!client.TryGet(prop.Name, type.GetDefaultValue(), out var value))
                {
                    continue;
                }
                var v = ValueUtils.ChangeToType(value, prop.PropertyType, new object());
                try
                {
                    prop.SetValue(dest, v);
                }
                catch (Exception)
                {
        ...
        {
            Type type = obj.GetType();
            PropertyInfo? property = type.GetProperty(propertyName) ?? throw new ArgumentException(string.Format("找不到属性'{0}'.", propertyName), propertyName);
